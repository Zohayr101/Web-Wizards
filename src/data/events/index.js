...
const register = async ({sql, getConnection}) => {
	...
	const addEvent = async ({userId, title, description, startDate, startTime, endDate, endTime, complete}) => {
		const cnx = await getConnection();
		const request = await cnx.request();
		request.input("userId", sql.VarChar(50), userId);
    ...
    const updateEvent = async ( { id, userId, title, description, startDate, startTime, endDate, endTime, complete } ) => {
		const cnx = await getConnection();
		const request = await cnx.request();
